  ...
  while(1)
  {
    if(UpdateAttitude())
    {
      ...
      if(First_Time)
      {
        ...
        rotate_angle=(encoderPosR-encoder_temp)/CPR*PI*2*TIRE_RADIUS/16.2/PI*180;//計算兩輪車的轉動角度
        if(millis()>9990&&millis()<10010)//原地平衡10秒後開始轉動
        {
            encoder_temp=encoderPosR;//紀錄當前encoder的值
        }
        if(millis()>10000&&encoder_flag==false)
        {
           Angle_Car_Adjustment=x-1;//讓兩輪車開始轉動
           if(rotate_angle>180)//當向右轉動大於180度時，停止轉動
           {
              Angle_Car_Adjustment=x;//讓兩輪車在原地平衡
              encoder_flag=true;
           }
        }
