  ...
  while(1)
  {
    if(UpdateAttitude())
    {
      ...
      if(First_Time)
      {
        ...
        if(c==1&&d==0)
          {
              if(millis()-t>4990&&millis()-t<5010)//原地平衡5秒
             {
                encoder_temp=encoderPosL;//記錄左輪的encoder值(用來計算左輪接下來要旋轉的角度)
                rotate_angle_l=0;//把左輪轉角度設為0
             }
             if(millis()-t>5000&&encoder_flag==false)//原地平衡5秒
              {
              Angle_Car_Adjustment=1;//當angle_car_adjustment為1時，會以比較快的速度向前移動
              if(rotate_angle_l>90)//判斷轉動角度是否大於90度
              {
                Angle_Car_Adjustment=x;//把angle_car_adjustment調整為初始的x值，使其平衡
          ...
          if(distance>60&&c==2&&d==0)//判斷是否達到移動的目標
          {
            Angle_Car_Adjustment=x;//把angle_car_adjustment調整為初始的x值，使其平衡
            distance=0;//把距離重設為0
            c=3;//準備執行下一個動作
          ...
          if(c==3&&d==0)
          {
              if(millis()-t>4990&&millis()-t<5010)//在原地平衡5秒
             {
                encoder_temp=encoderPosR;//記錄右輪的encoder值
                rotate_angle_r=0;//將右輪的轉動角度歸零
             }
             if(millis()-t>5000&&encoder_stop==false)//在原地平衡5秒
              {
              Angle_Car_Adjustment=1;//當angle_car_adjustment為1時，會以比較快的速度向前移動
              if(rotate_angle_r>90)//判斷轉動角度是否大於90度
              {
                Angle_Car_Adjustment=x;//把angle_car_adjustment調整為初始的x值，使其平衡
          ...
          if(distance>200&&c==4&&d==0)//判斷已經抵達目標
          {
            Angle_Car_Adjustment=x;//把angle_car_adjustment調整為初始的x值，使其平衡
