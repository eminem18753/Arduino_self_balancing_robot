...
void loop() {
  ...
  while(1)
  {
    if(UpdateAttitude())
    {
      ...
      {
        if(pwm_l>100&&flag_stop==0)//若pwm大於100，則代表兩輪車已經被推動，並確保其位處於平衡狀態
        {
                flag_pwm=1;//準備開始計算距離
                Angle_Car_Adjustment=5;//讓兩輪車以一定的速度向forward方向(使速度不要太快)
        }
        if(flag_pwm==1)
        {
                //check=20;
                distance=distance+Speed_LR*0.01;//計算兩輪車移動的距離
               if(pwm_l<15)//當兩輪車速度慢下來時
               {
                    flag_pwm=0;//兩輪車停止移動
                    flag_stop=10;//兩輪車準備移動回原出發位置
               }
        }
        if(flag_stop==10)
        {
             if(millis()%1000<100)//每1秒當中有0.1秒是以比較慢的速度在原地平衡
          {
              Angle_Car_Adjustment=x;//每1秒當中有0.1秒是以比較慢的速度在原地平衡
          }
          else if(millis()%1000>=100)//每1秒當中有0.9秒是在以比較快的速度在前進
          {
              Angle_Car_Adjustment=6;//當angle_car_adjustment為6時，會以比較快的速度前進
          }
          distance=distance+Speed_LR*0.01;//計算移動的距離
            if(distance<-10)//當距離小於-10公分時
            {
                flag_stop=0.3;//改變flag_stop，使兩輪車在原地平衡
                Angle_Car_Adjustment=x;//使兩輪車在原地平衡
            }
        }
         SendToPC(100);    
        PWM_Calculate();
