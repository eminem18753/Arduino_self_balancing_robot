  ...
  while(1)
  {
    if(UpdateAttitude())
    {
      ...
      if(First_Time)
      {
        ...
        if(millis()>3000)//先讓兩輪車原地平衡3秒
        {
          z=millis();//z是一個計時器，用來判斷執行完動作的平衡時間(使兩輪車更加穩定)
          distance=distance+Speed_LR*0.01;//計算移動的總距離
          if(flag_stop==false)//用來兩輪車是否已經抵達終點
          {
            if(pwm>=200)//若兩輪車速度太快，則先讓其回到原地平衡
           {
          ...
          {
            if((z-t)>=300)//當原地平衡了0.3秒之後，便可以開始繼續做動作
            {
        ...
        if(distance>300)//當距離到達300公分時，並停止移動
        {
          Angle_Car_Adjustment=x;//把angle_car_adjustment調整為初始的x值，使其平衡
          flag_stop=true;//改變flag_stop，使兩輪車維持原地平衡狀態
        }
        }
        PWM_Calculate();
