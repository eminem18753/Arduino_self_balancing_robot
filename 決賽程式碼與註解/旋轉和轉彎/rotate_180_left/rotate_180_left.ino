  ...
  while(1)
  {
    if(UpdateAttitude())
    {
      ...
      if(First_Time)
      {
        ...
        rotate_angle=(encoderPosL-encoder_temp)/CPR*PI*2*TIRE_RADIUS/16.2/PI*180;//計算兩輪車旋轉的角度
        if(millis()>4990&&millis()<5010)//當原地平衡五秒後開始準備轉動
        {
            encoder_temp=encoderPosL;//計算當前encoder的值
        }
        if(millis()>5000&&encoder_flag==false)
        {
           Angle_Car_Adjustment=1;//讓兩輪車向前傾斜
           if(rotate_angle>180)//當兩輪車向左轉動大於180度時，讓其停止轉動
           {
              Angle_Car_Adjustment=x;//把angle_car_adjustment調整為初始的x值，使其平衡
              encoder_flag=true;//讓兩輪車停止轉動
           }
        }
        PWM_Calculate();
