  ...
  while(1)
  {
    if(UpdateAttitude())
    {
      ...
      if(First_Time)
      {
        ...
        //Angle_Car = 0;
        First_Time = false;
        x=-AngleAvg;//x為angle_car_adjustment的初始值
        //temp=Angle_Car_Adjustment;
      }
      
      else if(AngleAvg < 45 || AngleAvg > -45){
        a=(int)s;//a是我們用手機來傳輸指令的參數
        ...
        switch(a)
        {
          ...
          case 1:         //forward
            ...
            {
              if((z-t)>=1500)//當原地平衡了1.5秒之後，便可以開始繼續做動作
              {
                if(millis()%1000<100)//每1秒當中有0.1秒是以比較慢的速度在前進
                {
                  Angle_Car_Adjustment=x+1;//當angle_car_adjustment為x+1時，兩輪車會朝forward方向前進
                }
                else if(millis()%1000>=100)//每1秒當中有0.9秒是在以比較快的速度在前進
                {
                  //Angle_Car_Adjustment=x-3.5;
                  Angle_Car_Adjustment=6.5;//當angle_car_adjustment為6.5時，會以比較快的速度向前移動
                }
              }
            }
            break;
          case 2:         //backward
            ...
            else
            {
              if((z-t)>=1500)//當原地平衡了1.5秒之後，便可以開始繼續做動作
              {
                if(millis()%1000<100)//每1秒當中有0.1秒是以比較慢的速度在前進
                {
                  Angle_Car_Adjustment=x-1;//每1秒當中有0.1秒是以比較慢的速度在後退
                }
          ...
          case 3:         //forward
            ...
            else
            {
              if((z-t)>=1500)//當原地平衡了1.5秒之後，便可以開始繼續做動作
              {
                if(millis()%1000<100)//每1秒當中有0.1秒是以比較慢的速度在前進
                {
                  Angle_Car_Adjustment=x+1;//每1秒當中有0.1秒是以比較慢的速度在前進
                }
                else if(millis()%1000>=100)//每1秒當中有0.9秒是在以比較快的速度在前進
                {
                  //Angle_Car_Adjustment=x-3.5;
                  Angle_Car_Adjustment=6.8;//當angle_car_adjustment為6.8時，會以比較快的速度前進
                }
              }
            }
            break;
          case 4:         //backward 並校正兩輪速度
            if(pwm>=180)//當pwm值太大時，使車子能盡量保持平衡狀態
            {
              Angle_Car_Adjustment=x;//把angle_car_adjustment調整為初始的x值，使其平衡
              t=millis();//計算所需要的平衡時間
